...
#if defined(NDEBUG)
# error "VapeX cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // VapeX: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "VapeX Signed Message:\n";
...
{
    // VapeX: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // VapeX: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // VapeX: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // VapeX: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // VapeX: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // VapeX: Special short-term limits to avoid 10,000 BDB lock limit:
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // VapeX: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VapeXMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VapeXMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static VapeXMiner(CWallet *pwallet)
{
    printf("VapeXMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("VapeX-miner");
    ...
    try { loop {
        ...
        printf("Running VapeXMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VapeXMiner, pwallet));
}
